...
export function UserList({ user }) {
  ...
  useEffect(() => {
    const fetchAssignedIds = async () => {
      try {
        const fetchedAssignedIds = await userService.getById(user.id)
  ...
  useEffect(() => {
    const fetchAssignedInfo = async () => {
      try {
        if (user.role === 'manager') {
          const fetchedAssignedAgents = await Promise.all(user.assigned_agents.map(id => userService.getById(id)))
    ...
    return (
      <div className="user-list">
        <div className="user-list-container">
          <div className={`selects-container ${user.role === "agent" ? "centered" : "two-columns"}`}>
          ...
          {user.role === 'agent' && selectedSite && <ProductList selectedSite={selectedSite} />}
          {user.role === 'manager' && selectedUser && selectedSite && <ProductList user={selectedUser} selectedSite={selectedSite} />}
        </div>
      </div >
